// GravityZone's getQuarantineItemsList has no "empty page" response for a page beyond the
...
// without this, the fetch would end in a hard failure on its final "one past the end"
// page, discarding rows already collected from earlier pages. Treat that specific,
// expected error as "no more results" (paired with postRequestScript, which turns the
// same suppressed response into an empty row array). Any other error (auth, rate limit,
// etc.) still surfaces normally.
